            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <?php
                        $users = mysqli_query($koneksi, "SELECT * FROM users WHERE username != '$user_aktif' ORDER BY id DESC");
                        ...
                        <tr class="group hover:bg-blue-50/30 transition">
                            <td class="p-6">
                                ...
                                <p class="text-[8px] font-bold text-slate-300 uppercase italic mt-1">User ID: #<?php echo $u['id']; ?></p>
                            </td>
                            ...
                            <td class="p-6">
                                <form method="POST" class="flex items-center justify-center gap-2">
                                    ...
                                    <select name="role" class="p-2.5 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-bold uppercase outline-none focus:bg-white focus:ring-2 focus:ring-blue-500 transition appearance-none min-w-[140px]">
                                        <option value="Pending" <?php if($u['role']=='Pending') echo 'selected'; ?>>Pending</option>
                                        <option value="Manager Gudang" <?php if($u['role']=='Manager Gudang') echo 'selected'; ?>>Manager</option>
                            ...
                            <td class="p-6 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="admin_users.php?hapus=<?php echo $u['id']; ?>" onclick="return confirm('Hapus user ini?')" class="w-9 h-9 flex items-center justify-center bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white transition rounded-xl shadow-sm">
